...
// /^\+[0-9]{3}\s\((\d+)\)-\d{3}-\d{2}-\d{2}/ (+380 (**)-***-**-**)
//or ^ [(][\d]{ 3 } [)]\s[\d]{ 3 } [-][\d]{ 4 }
//or +? d{ 1, 4 }?[-.s] ? (? d{ 1, 3 }?)?[-.s] ? d{ 1, 4 } [-.s] ? d{ 1, 4 } [-.s] ? d{ 1, 9 }

const patternId = "\\w{8}-\\w{4}-\\w{4}-\\w{4}-\\w{12}";

const achemaContact = Joi.object({
...
const validate = async (schema, obj, res, next) => {
  try {
    ...
    res.status(400).json({status: "error", code: 400, message: `Field ${err.message.replace(/"/g, "")}`});
  }
};

module.exports.validateContact = async (req, res, next) => {
    return await validate(achemaContact, req.body, res, next)
}
